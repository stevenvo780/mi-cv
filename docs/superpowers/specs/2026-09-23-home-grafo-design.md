...
- **Home:** cada catálogo abre su frente, antes de las tarjetas: es un trabajo más del frente, uno que reúne otros. Cada uno es una tarjeta con el color de su marca, un borde de luz que la recorre (`--cat-angle`, como el de «Pregúntame»), dos hojas apiladas detrás que se abren en abanico con el puntero o el foco, el rótulo «Catálogo · N …», su lema, su descripción y **un mapa de lo que reúne** (2026-09-26, abajo). Cada ítem con URL enlaza directo.
- **Por qué no en una banda aparte (2026-09-25):** hasta esta fecha iban en una banda «Catálogos» encima de los frentes, cada tarjeta rotulada «§ NN · Frente». Eran cuatro tarjetas, una por frente, seguidas de cuatro secciones con el mismo § y el mismo nombre: dos listas con la misma clave se leen como resumen y detalle, así que cada frente parecía el desarrollo de su catálogo. No lo es: lo que reúnen Kósmos, Daímon y Paideía no está en las tarjetas de su frente (solo el repositorio de la tesis coincide), y las tarjetas del frente no están en su catálogo. Dentro del frente, el catálogo es un hermano de las demás tarjetas, no su cabecera, y el rótulo del frente sobra. Humanizar es otra cosa: la vitrina comercial de Humanizar Systems, y sí lista productos que tienen tarjeta propia en varios frentes.
- **El mapa (2026-09-26):** el muro de listas ocupaba unos 820 px por catálogo; ahora es un grafo, como el del hero. Con 51.5rem de tarjeta o más (ventanas desde 1 280 px) es un mapa: el nombre del catálogo en el centro, delante de su cifra en contorno; sus colecciones repartidas a los dos lados (reparto voraz por filas) con un nodo en rombo; sus ítems en dos columnas de etiquetas, cada uno con su nodo; y aristas en curva del centro a cada colección y de cada colección a sus ítems (un `<svg>` 100×100 estirado, `preserveAspectRatio="none"` y `vector-effect: non-scaling-stroke`). Las posiciones las calcula `src/lib/catalogMap.ts` en el servidor y llegan como variables CSS (`--y`, `--hy`, `--xi`, `--xg`): sin JS nuevo. Una etiqueta puede ocupar dos líneas; cuántas se mide con la tabla de avances de Geist (`src/lib/generated/geistAdvance.ts`, que genera `scripts/build-text-widths.mts`) y el ancho de la columna en el mapa más estrecho, y `tests/lib/catalog-map.test.ts` lo contrasta con fontkit (con kerning). Con movimiento, las aristas se abren desde el centro mientras el mapa entra (`view-timeline` del mapa y `clip-path`; `stroke-dasharray` con `non-scaling-stroke` no medía bien) y las recorren pulsos; pasar por una colección la enciende y apaga las demás. En una tarjeta más estrecha, el mapa se pliega en un árbol por colección (rombo, espina y un nodo por ítem), en columnas o en carrusel por debajo de 600 px. Kósmos pasa de ~820 a ~700 px de alto en escritorio; Daímon, de ~820 a ~610.
