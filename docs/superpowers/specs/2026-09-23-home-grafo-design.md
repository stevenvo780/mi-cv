...
- **Home:** van en una banda propia, «Catálogos», al principio del catálogo y fuera de la rejilla de su frente (la cabecera del frente enlaza a su tarjeta). Cada uno es una tarjeta ancha con el color de su marca, un borde de luz que la recorre (`--cat-angle`, como el de «Pregúntame»), dos hojas apiladas detrás que se abren en abanico con el puntero o el foco, el rótulo «Catálogo · N …», la cifra en contorno y la lista entera de lo que contiene, por colección (columnas en escritorio, carrusel horizontal por debajo de 600 px). Cada ítem con URL enlaza directo.
...
- **Buscador:** el `data-search` de cada tarjeta lleva los títulos de sus ítems y los nombres de sus colecciones; la banda tiene `data-front`, así que `applySearchFilter` la oculta sin cambiar su código.
...
- **El puntero sobre una tarjeta es contenido:** `.cat` y `.cats-head` van en `CONTENT` de `GraphStage` (chunk diferido del 3D), así que ni el hover ni un clic en su fondo llegan al grafo. Lo comprueba un e2e del proyecto `3d`, que pone la tarjeta y la cabecera delante de un nodo (falla sin el cambio).
- **Grafo:** cada catálogo pesa 4, como un frente, y se une (`agrupa`) a los productos del portafolio que reúne, los que comparten sitio o repositorio con uno de sus ítems (hoy Paideía y Kósmos → Estructuras Preontológicas): 179 nodos y 264 aristas.
