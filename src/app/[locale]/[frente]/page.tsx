'use client';
/**
 * /[locale]/[frente] — Per-frente product landing
 *
 * Shows the gallery of products for a single frente.
 * Valid frentes: filosofia | ciencias | informatica | enterprise
 * Invalid frente → notFound()
 *
 * Does NOT collide with /[locale]/lore — static segments take priority.
 */
...
export default function FrentePage() {
  ...
  const frenteParam = params?.frente as string;

  if (!VALID_FRENTES.includes(frenteParam as FrenteId)) {
    notFound();
  }

  const frenteId = frenteParam as FrenteId;
