...
function HubCard({ label, sub, url, accent = 'teal', delay = 0, internal = false }: HubCardProps) {
  const colors = {
    gold: { border: 'rgba(224,168,94,0.22)', hover: 'rgba(224,168,94,0.40)', text: 'var(--gold)', dot: '#e0a85e' },
    teal: { border: 'rgba(67,181,166,0.22)', hover: 'rgba(67,181,166,0.40)', text: 'var(--teal)', dot: '#43b5a6' },
    rust: { border: 'rgba(207,106,60,0.22)', hover: 'rgba(207,106,60,0.40)', text: 'var(--rust)', dot: '#cf6a3c' },
  }[accent];

  const sharedStyle = {
    animationDelay: `${delay}ms`,
    '--hub-border': colors.border,
    '--hub-hover': colors.hover,
    '--hub-text': colors.text,
    '--hub-dot': colors.dot,
  } as React.CSSProperties;
  ...
  if (internal) {
    return (
      <Link href={url} className="hub-card reveal" data-accent={accent} style={sharedStyle}>
        {inner}
      </Link>
    );
  }
...
function CatLabel({ children }: { children: React.ReactNode }) {
  return <p className="lt-cat-label">{children}</p>;
}
