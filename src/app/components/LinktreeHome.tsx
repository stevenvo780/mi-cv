...
import { productos, frentesMeta, productTags } from '@/data/frentes';
...
const T = {
  es: {
    ...
    links: {
      ...
      cauce:       { label: 'Cauce V3 · Flagship', sub: 'humanizar.tech/cauce-v3',     url: 'https://humanizar.tech' },
...
export default function LinktreeHome() {
  ...
  // When searching, force filtered results visible: the scroll-reveal observer
  // only runs on mount, so newly-rendered filtered sections would stay hidden.
  React.useEffect(() => {
    if (query.trim()) {
      document
